...
#   @micropython.native
  def _draw( self, aPixels ) :
    ...
    for i in range(aPixels//32):
      ...
      print("WDATA", binascii.hexlify(self.buf))
    rest = (int(aPixels) % 32)
    if rest > 0:
        buf2 = bytes(self.colorData) * rest
        self.spi.write(buf2)
        print("WDATA", binascii.hexlify(buf2))
        print("WDATA", binascii.hexlify(buf2))
    self.cs(1)
...
#   @micropython.native
  ...
  def _writecommand( self, aCommand ) :
    ...
    print("CMD", binascii.hexlify(bytearray([aCommand])))
    self.cs(1)
  ...
  def _writedata( self, aData ) :
    ...
    print("DATA", binascii.hexlify(bytearray(aData)))
    self.cs(1)
